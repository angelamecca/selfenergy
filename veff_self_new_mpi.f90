! This program compute the single-particle properties of
...
program veff_PNM
 ...
 implicit none 
  ...
  real(kind = r8), parameter :: small= 1.e-9
  ...
  if(polchoice.eq.1) then
     ...
     call bcast_vect_1d(lambda)
     
     write(polnum,'(I3.3)') int(lambda(3)*100.d0 )
     namepol = "pol_ek/"//trim(polnum)//"/"
     if(lambda(3).eq.0..or.lambda(3).eq.1. ) then
        nu = 1
     else
        nu = 2
     end if
  ...
  !the master reads the table of the effective  interaction in 'Data/veffmat.data'
  ...
  if(myid.eq.master) then
     ...
     write(2,*) '# rho, kF, TF, e1, etot, (E-TF)/TF'
     open(72,file=trim(namepol)//'etot2.dat')
     write(72,*) '# rho, kF, TF, e1, etot, (E-TF)/TF'
  ...
  do irho =  1 , irhomax
     ...
     if(myid.eq.master) then
        ...
        write(*,*)'kF  = ',kF, 'kF_i  =', kF_i(1), kF_i(2), kF_i(3), kF_i(4)
        write(*,*)'rho = ',rho, 'rho_i =', rho_i(1), rho_i(2), rho_i(3), rho_i(4)
        ...
        write(*,*)
        write(*,*) '======== Total energy ========'
        write(*,*) 'rho, kF, TF, V, etot  (E-TF)/TF' 
        write(*,'(1p,7e14.4)')  rho, kF, TF, e1      , etot      , (e1)/TF
        write(*,'(1p,7e14.4)')  rho, kF, TF, e1_check, etot_check, (e1_check)/TF
        write(*,*)'============================'
        write(*,*)
        
        write(2,'(1p,6e14.4)')  rho, kF, TF,       e1      , etot      , (e1)/TF
        write(72,'(1p,6e14.4)') rho, kF, TF_check, e1_check, etot_check, (e1_check)/TF
