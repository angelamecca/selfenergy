...
module vegas_module
  ...
  integer, parameter :: nq = 1000   ! poi vedi 
  integer :: nmax_par
  real(kind=my_kind), parameter :: alpha_cut = 12.5d0
contains
...
subroutine real_SE(ReSE,ReSE_er,EE,nE,kk,nk,dove,failed,mass,kF,htc,ik_in, ik_fin, in_in, in_fin)
  use Boundary
  use vegas_module
  !use precision
  ...
  parameter (seed = 0)
  parameter (mineval = 0)
  ! parameter (maxeval =  1000000000) !!!!!!cambiare
  parameter (maxeval =  100000)  
  integer nstart, nincrease, nbatch, gridno
  character*(*) statefile
  ! parameter (nstart = 50000)
   parameter (nstart = 5000)
  ...
  real(kind=r8) :: const
  integer       :: in, ik, nE, nk,ik_in, ik_fin, in_in, in_fin
  real(kind=r8) :: kcut, kpole
  real(kind=r8) :: ReSE(nE,nk), ReSE_er(nE,nk),  EE(nE), kk(nk) !,  dE(nE)
  integer       :: failed(nE,nk)
  logical       :: im
  real(kind=r8) :: pi, mass, htc, kF, EkF, Ene
  character(len=64)  :: dove, filenum, nameV

   if(ik_in.lt.1.or.ik_fin.gt.nk) then
       write(*,*) ' out of range: ik_in > 1,  ik_fin > nk', ik_in, ik_fin, nk
       stop
    end if

    if(in_in.lt.1.or.in_fin.gt.nE) then
       write(*,*) ' out of range: in_in > 1,  in_fin > nE', in_in, in_fin, nE
       stop
    end if
  ...
  do ik = ik_in, ik_fin
     ...
     do in = in_in, in_fin
        ...
        if(part) then
           ...
           write(18,155) const*integral1(1),const*error1(1),fail1,kk(ik)/kF,Ene/EkF,neval1,kpole
           flush(18)
           ...
           write(19,155) const*integral2(1),const*error2(1),fail2,kk(ik)/kF,Ene/EkF,neval2,kpole
           flush(19)
           ...
           !print *, " "
           write(20,155) const*integral3(1),const*error3(1),fail3,kk(ik)/kF,Ene/EkF,neval3,kpole
           flush(20)
           !                                                     
           ReSE(in, ik) = const*(integral1(1)+2.d0*integral2(1)+integral3(1))
           ReSE_er(in, ik) = const*sqrt(error1(1)**2+error2(1)**2+error3(1)**2)
           Failed(in,ik) = fail1 + fail2 + fail3
           neval     = neval1+ neval2 + neval3
           
          ! write(6,156) ReSE(in, ik),ReSE_er(in,ik),Failed(in,ik),kk(ik)/kF,Ene/EkF,neval           
           write(17,156) ReSE(in,ik),ReSE_er(in,ik),Failed(in,ik),kk(ik)/kF,Ene/EkF,neval
           flush(17)     
           
           !----------------> HOLES  
        else
           write(*,*)
           write(*,*) 'REAL holes', kk(ik), ene,  ik,  in
           write(*,*)
           lowerIM(1) =  0.d0
           upperIM(1) =  kF
           lowerIM(2) =  0.d0
           ...
           ReSE(in, ik) = const*integral1(1)
           ReSE_er(in,ik)= const*error1(1)
           Failed(in,ik)= fail1
           neval = neval1
           write(17,156) ReSE(in, ik),ReSE_er(in,ik),Failed(in,ik),kk(ik)/kF,Ene/EkF,neval
           flush(17)
...
!========================================================================================
 ...
 subroutine imaginary_SE(ImSE,ImSE_er,EE,nE,kk,nk,dove,failed,mass,kF,htc,ik_in, ik_fin,in_in, in_fin)      
    use Boundary
    use vegas_module
    !use precision
   ...
   interface
    ...
    integer       :: in, ik, nE, nk, in_in, in_fin, ik_in, ik_fin
    real(kind=r8) :: ImSE(nE,nk),ImSE_er(nE,nk), EE(nE), kk(nk)! , the_err(nE)
  ...
  do ik = ik_in,  ik_fin
     ...
     do in = in_in, in_fin
       ...
       ! write(*,*) 'ene/ekf', Ene/EkF, in, pp_csieta, 'pmin, pmax', pmin/kF, pmax/kF
        ...
        !     print *, " "
        IMSE(in, ik)    = const*integral(1)
        IMSE_er(in, ik) = const*error(1)
        Failed(in,ik)   = fail
        write(17,156) ImSE(in, ik),ImSE_er(in,ik),Failed(in,ik),kk(ik)/kF,Ene/EkF,neval
        flush(17)     
     end do
     close(17)
